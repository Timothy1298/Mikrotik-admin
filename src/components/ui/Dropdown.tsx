...
export function Dropdown({ items }: { items: Array<{ label: string; onClick?: () => void; danger?: boolean }> }) {
  return (
    <div className="group relative inline-block text-left" onClick={(event) => event.stopPropagation()}>
      ...
      <div className="absolute right-0 z-20 mt-2 hidden min-w-44 rounded-2xl border border-brand-500/15 bg-[rgba(8,14,31,0.9)] p-2 group-focus-within:block group-hover:block">
        {items.map((item) => (
