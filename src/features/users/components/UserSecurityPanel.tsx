      ...
      <Card>
        ...
        <div className="space-y-4">
          {flags.length ? flags.map((flag, index) => (
            <div key={`${flag.flag}-${index}`} className="rounded-2xl border border-background-border bg-background-panel p-4">
              <div className="flex items-center justify-between gap-3"><p className="font-medium text-text-primary">{flag.flag}</p><UserStatusBadge status={flag.severity === 'high' ? 'flagged' : 'watchlist'} /></div>
              <p className="mt-2 text-sm text-text-secondary">{flag.description || 'No description provided.'}</p>
              <p className="mt-2 font-mono text-xs text-text-muted">{flag.createdBy} • {formatDateTime(flag.createdAt)}</p>
            </div>
          )) : <p className="text-sm text-text-secondary">No risk flags recorded for this account.</p>}
        </div>
      </Card>
    </div>
  );
