      ...
      <Card>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[640px] text-left text-sm">
            <thead className="font-mono text-text-muted"><tr><th className="pb-3">Reference</th><th className="pb-3">Type</th><th className="pb-3">Amount</th><th className="pb-3">Status</th><th className="pb-3">Created</th></tr></thead>
            <tbody className="divide-y divide-brand-500/15">
              {transactions.length ? transactions.map((transaction) => (
                <tr key={transaction.id}>
                  <td className="py-4 font-mono text-text-primary">{transaction.transactionId}</td>
                  <td className="py-4 text-text-secondary">{transaction.type}</td>
                  <td className="py-4 text-text-secondary">{formatCurrency(transaction.amount, transaction.currency)}</td>
                  <td className="py-4"><SubscriptionStatusBadge status={transaction.status} /></td>
                  <td className="py-4 font-mono text-text-secondary">{formatDateTime(transaction.createdAt)}</td>
                </tr>
              )) : (
                <tr>
                  <td colSpan={5} className="py-6 text-center text-sm text-text-muted">No invoices or payment records available for this account.</td>
                </tr>
              )}
            </tbody>
          </table>
