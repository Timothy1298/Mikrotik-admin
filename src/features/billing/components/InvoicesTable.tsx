...
export function InvoicesTable({ rows, onOpen, onDownloadPdf }: { rows: BillingTransaction[]; onOpen: (row: BillingTransaction) => void; onDownloadPdf?: (row: BillingTransaction) => void }) {
  const columns = useMemo<ColumnDef<BillingTransaction>[]>(() => [
    { header: "Invoice", cell: ({ row }) => <div><p className="font-medium text-text-primary">{row.original.transactionId}</p><p className="text-xs text-text-muted">{row.original.description}</p></div> },
    ...
    { header: "Due", cell: ({ row }) => <span className="font-mono text-xs text-text-secondary">{formatDateTime(row.original.dueDate)}</span> },
