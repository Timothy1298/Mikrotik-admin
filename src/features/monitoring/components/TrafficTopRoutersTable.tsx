...
export function TrafficTopRoutersTable({ rows, onOpen }: { rows: TrafficRouterRow[]; onOpen: (row: TrafficRouterRow) => void }) {
  const columns = useMemo<ColumnDef<TrafficRouterRow>[]>(() => [
    ...
    { header: "Ingress", cell: ({ row }) => <span className="text-sm text-slate-200">{formatBytes(row.original.transferRx)}</span> },
    { header: "Egress", cell: ({ row }) => <span className="text-sm text-slate-200">{formatBytes(row.original.transferTx)}</span> },
    { header: "Total", cell: ({ row }) => <span className="font-medium text-slate-100">{formatBytes(row.original.totalTransferBytes)}</span> },
  ], []);
