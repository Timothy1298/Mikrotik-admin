        ...
        <div className="space-y-4">
          <Card>
            ...
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              <div className="rounded-2xl border border-background-border bg-background-panel p-4 text-sm text-text-primary">Awaiting: {detail.ticket.awaitingState.replace(/_/g, " ")}</div>
              ...
              <div className="rounded-2xl border border-background-border bg-background-panel p-4 text-sm text-text-primary">Age: {detail.ticket.age.ageHours}h · Idle {detail.ticket.age.idleHours}h</div>
              <div className="rounded-2xl border border-background-border bg-background-panel p-4 text-sm text-text-primary">Linked resources: {[detail.context.router, detail.context.vpnServer, detail.context.incident, detail.context.subscription, detail.context.transaction].filter(Boolean).length}</div>
            </div>
          </Card>
          ...
          <Card>
            ...
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              <div className="rounded-2xl border border-background-border bg-background-panel p-4 text-sm text-text-primary">Customer: {detail.context.customer?.name || "Unknown"} · {detail.context.customer?.supportTier || "standard"}</div>
              <div className="rounded-2xl border border-background-border bg-background-panel p-4 text-sm text-text-primary">Router: {detail.context.router?.name || "None linked"}</div>
              <div className="rounded-2xl border border-background-border bg-background-panel p-4 text-sm text-text-primary">VPN server: {detail.context.vpnServer?.name || "None linked"}</div>
              <div className="rounded-2xl border border-background-border bg-background-panel p-4 text-sm text-text-primary">Incident: {detail.context.incident?.title || "None linked"}</div>
            </div>
          </Card>
              ...
              {messagesQuery.isPending ? <SectionLoader /> : (messagesQuery.data?.items || []).length ? messagesQuery.data?.items.map((message) => (
                <div key={message.id} className={`rounded-2xl border p-4 ${message.direction === "admin" ? "border-primary/40 bg-primary/10" : "border-background-border bg-background-panel"}`}>
                  <div className="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between sm:gap-3">
                    <p className="break-words text-sm font-medium text-text-primary">{message.author?.email || message.source}</p>
                    <p className="shrink-0 text-xs text-text-muted">{formatDateTime(message.createdAt)}</p>
                  </div>
                  <p className="mt-2 text-sm text-text-secondary">{message.body}</p>
                  {message.attachments.length ? <div className="mt-3 flex flex-wrap gap-2">{message.attachments.map((attachment) => <span key={attachment.url} className="rounded-full border border-background-border px-3 py-1 text-xs text-text-secondary">{attachment.filename}</span>)}</div> : null}
                </div>
              ...
              <div className="space-y-3">
                {(flagsQuery.data || []).length ? flagsQuery.data?.map((flag) => (
                  <div key={flag.id} className="rounded-2xl border border-background-border bg-background-panel p-4">
                    ...
                    <p className="mt-2 text-xs text-text-muted">{flag.severity} · {flag.description || "No description"}</p>
                  </div>
                )) : <EmptyState icon={LifeBuoy} title="No flags" description="This ticket does not currently have any internal workflow flags." />}
                <div className="border-t border-background-border pt-3">
