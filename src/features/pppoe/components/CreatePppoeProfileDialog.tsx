...
import type { PppoeProfilePayload } from "@/features/pppoe/types/pppoe.types";
...
export function CreatePppoeProfileDialog({
  ...
  return (
    <Modal open={open} onClose={() => { reset(); onClose(); }} title="Create PPPoE profile" description='Define a PPPoE access profile. Rate limit format example: "10M/5M".'>
      <form className="space-y-4" onSubmit={handleSubmit((values) => onConfirm(values))}>
        ...
        <div className="flex justify-end gap-3">
          ...
          <Button type="submit" isLoading={loading}>Create profile</Button>
        </div>
      </form>
    </Modal>
  );
