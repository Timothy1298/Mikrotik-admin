...
export function HotspotSessionsTable({
  ...
  const columns = useMemo<ColumnDef<HotspotSession>[]>(() => [
    ...
    { header: "IP address", cell: ({ row }) => <span className="font-mono text-sm text-text-secondary">{row.original.ip || "Unknown"}</span> },
    { header: "MAC", cell: ({ row }) => <span className="font-mono text-sm text-text-secondary">{row.original.mac || "Unknown"}</span> },
    { header: "Upload", cell: ({ row }) => <span className="text-sm text-text-primary">{formatBytes(row.original.uplinkBytes || 0)}</span> },
    { header: "Download", cell: ({ row }) => <span className="text-sm text-text-primary">{formatBytes(row.original.downlinkBytes || 0)}</span> },
    {
      header: "Session duration",
      cell: ({ row }) => (
        <div className="space-y-1">
          <p className="text-sm text-text-primary">{formatDuration(row.original.startedAt)}</p>
