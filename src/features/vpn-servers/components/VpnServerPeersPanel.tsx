            ...
            <div key={peer.id} className="rounded-2xl border border-brand-500/15 bg-[rgba(8,14,31,0.9)] p-4">
              <div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-slate-500">↓ {formatBytes(peer.transferRx)} / ↑ {formatBytes(peer.transferTx)}</p>
                  <p className="text-xs text-slate-400">
                    {peer.router ? <Link className="transition hover:text-slate-100" to={appRoutes.routerDetail(peer.router.id)}>{peer.router.name}</Link> : "No linked router"}
                  </p>
                </div>
                <p className="text-xs text-slate-500">{peer.lastHandshake ? dayjs(peer.lastHandshake).fromNow() : "Never"}</p>
              </div>
            </div>
          );
